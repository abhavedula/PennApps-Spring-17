...
public class Ngram {
    ...
    public void update(String sentence) {
        System.out.println("154" + ngrams(tokenize(sentence)));
    ...
    public String getMostLikelySugg(List<String> context, JSONArray sugg){
        ...
        String token = "";
        try {
            token = (String) sugg.get(0);
            double max = -1;

            for(int i = 0; i < sugg.length(); i++){
                if(prob(context, sugg.getString(i)) > max) {
                    max = prob(context, sugg.getString(i));
                    token = sugg.getString(i);
                }
                System.out.println("PROB: " + prob(context, sugg.getString(i)));
            }
